...
from conv import GNN_node, GNN_node_Virtualnode, GNN_node_cyclegan

from torch_scatter import scatter_mean


class GNN_Disciminator_CycleGAN(torch.nn.Module):
    ...
    def __init__(self, num_tasks = 1, num_layers = 5, emb_dim = 300, 
        ...
        '''
        super(GNN_Disciminator_CycleGAN, self).__init__()
        ...
        if virtual_node:
            ...
            self.gnn_node = GNN_node_cyclegan(num_layers, emb_dim, JK = JK, drop_ratio = drop_ratio, residual = residual, gnn_type = gnn_type)
...
class GNN_Generator_CycleGAN(torch.nn.Module):
    ...
    def __init__(self, num_tasks = 1, num_layers = 5, emb_dim = 300, 
        ...
        '''
            ...
            virtual_node (bool): whether to add virtual node or not
        '''
        super(GNN_Generator_CycleGAN, self).__init__()
    ...
    def forward(self, xyz, xyz_edge_index, xyz_edge_attr, batch ):
        ...
        if xyz.size(-1)==3:
            d2_pred=self.twod_emb_linear(h_node)

            if self.training:

                return output,d2_pred
            else:
                # At inference time, we clamp the value between 0 and 20
                return torch.clamp(output, min=0, max=20)
        elif xyz.size(-1)==9 or xyz.size(-1)==self.emb_dim:
