...
def xyzData(Data):
    def __init__(self,xyz=None,**kwargs):
        super(PygPCQM4Mv2Dataset_xyz, self).__init__(**kwargs)
        self.xyz=xyz
...
class PygPCQM4Mv2Dataset_xyz(InMemoryDataset):
    def __init__(self, root = 'dataset', smiles2graph = smiles2graph, transform=None, pre_transform = None):
        ...
        self.folder = osp.join(root, 'pcqm4m-v2_combine_xyz')
        self.version = 1
    ...
    def process(self):
        ...
        data_list = []
        xyzpathdict=readxyzpath("dataset/pcqm4m-v2_xyz")
        ...
        assert(all([torch.isnan(data_list[i].xyz)[0] for i in split_dict['test-dev']]))
        assert(all([torch.isnan(data_list[i].xyz)[0] for i in split_dict['test-challenge']]))
